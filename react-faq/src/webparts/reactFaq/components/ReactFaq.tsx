...
> {
  ...
  public onSuggestionSelected = (FaqData, event) => {
    const FaqFilteredData = this.filterByValue(
      FaqData,
  ...
  public categoryAndQuestionSorting = (Data) => {
    ...
    distCate.forEach((distCateItem) => {
      Data.map((item) => {
        if (
          distCateItem.Category.toLowerCase() == item.Category.toLowerCase()
        ) {
          result.push(item);
        }
      });
  ...
  public filterByValue = (arrayData, value) => {
    return arrayData.filter(
      (o) =>
        this.includes(o["Title"].toLowerCase(), value.toLowerCase()) ||
        this.includes(o["Answer"].toLowerCase(), value.toLowerCase())
    );
  };
  ...
  public loadMoreEvent(event: any): void {
    ...
    if (event.target.nodeName == "SPAN") {
      ...
      if (event.target.nodeName == "path") {
        ...
        if (
          event.target.parentElement.getAttribute("data-icon") == "plus-square"
        ) {
          event.target.parentElement.nextElementSibling.nextElementSibling.nextElementSibling.classList.remove(
            "hideDiv"
          );
          ...
          event.target.parentElement.nextElementSibling.nextElementSibling.classList.add(
            "hideDiv"
          );
          event.target.parentElement.nextElementSibling.nextElementSibling.removeAttribute(
            "style"
          );
          if (event.currentTarget.children[0].classList != undefined) {
            event.currentTarget.children[0].classList.remove("hideDiv");
        ...
        if (event.target.getAttribute("data-icon") == "plus-square") {
          ...
          if (event.target.nextElementSibling.classList != undefined) {
            ...
            // IE11 does not implement classList on <svg>
            let appliedClassesII =
              event.target.nextElementSibling.getAttribute("class") || "";
            appliedClassesII =
              appliedClassesII.split(" ").indexOf("hideDiv") != -1
                ? appliedClassesII.replace(" hideDiv", "")
                : appliedClassesII + " hideDiv";
          ...
          if (event.target.previousElementSibling.classList != undefined) {
            ...
            // IE11 does not implement classList on <svg>
            let appliedClassesII =
              event.target.previousElementSibling.getAttribute("class") || "";
            appliedClassesII =
              appliedClassesII.split(" ").indexOf("hideDiv") != -1
                ? appliedClassesII.replace(" hideDiv", "")
                : appliedClassesII + " hideDiv";
    ...
    return (
      <div className={`container`}>
        <div className="FaqSearchBox" accept-charset="UTF-8">
          <div className="position-zoom">
            <span className="dot"></span>
            <div className="gg-zoom-in"></div>
          </div>
          <Autosuggest
                      ...
                      <AccordionItem uuid={allCat.Category}>
                        ...
                        <AccordionItemPanel>
                          <div className="acc-item-panel">
                            {FaqData.filter(
                              (it) => it.Category == allCat.Category
                            ).map((allFaq) => (
                              <div
                                className="acc-item"
                                data-id={allFaq.Id}
                              ...
                              >
                                ...
                                />
                                <FontAwesomeIcon
                                  icon={fontawesome.faMinusSquare}
                                  size="1x"
                                  data-id={allFaq.Id}
