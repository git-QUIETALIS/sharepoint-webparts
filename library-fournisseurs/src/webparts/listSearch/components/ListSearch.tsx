...
export default class IListdSearchWebPart extends React.Component<IListSearchProps, IListSearchState> {
  ...
  private async getData() {
    try {
      ...
      // Triggered when component is fully loaded
      if (!this.state.isLoading) {
        this.changeFolderIcons();
      }
  ...
  public changeFolderIcons() {
    let fournisseurs = this._generateFournisseurs();

    // Change the default file icon to a folder icon for all items in the list
    fournisseurs.forEach(function (NomFournisseur) {
      // Get the div with the fournisseur name only if it exists in this page
      if ($("span:contains('" + NomFournisseur + "')").length > 0) {
        let fournisseurNameDiv = $("span:contains('" + NomFournisseur + "')");

        // Go back to the parent parent div
        fournisseurNameDiv = fournisseurNameDiv.parent().parent();
  ...
  private async readListsItems(): Promise<Array<any>> {
    ...
    result = this._RemoveUnwantedResults(result, fournisseurs);

    return result;
  }

  private _RemoveUnwantedResults(result: any[], fournisseurs: any[]) {
    // Iterate through result and console log each item
    result.forEach(item => {
      if (fournisseurs.includes(item.Nom)) {
        //console.log(item.Nom);
      } else {
        // Remove from result the item that item.Nom equal to item.Nom
        result = result.filter(e => e.Nom !== item.Nom);
      }
    });
